...
async function filterTool() {
    ...
    filterForm.addEventListener('submit', function (event) {
        ...
        if (filters.search) {
            ...
            Otherwise, with getPagePokes() we receive the first 25 Pokémons of the first (pageNumber) page,
            ...
            let statsFiltersExists = false;
            filters.stats.forEach(stat => {
                if (stat.maximum || stat.minimum) {
                    removePrevState(emptyStateContainer, pokeCards);
                    statsFiltersExists = true;
                    if (filteredPokemons.length <= 25) {
                        createPokeCards(filteredPokemons);
                    } else {
                        getInitialPage(filteredPokemons);
            ...
            if (statsFiltersExists === false) {
                /* If there is not search, if there are already filtered Pokémons displayed, and if there are no
...
function getPagePokes(pokemonList) {
    let actualPagePokes = [];
